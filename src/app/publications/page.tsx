...
export const metadata: Metadata = buildMetadata({
  title: "आकार IAS प्रकाशन – सिविल सेवा परीक्षा हेतु पुस्तकें",
  description: "UPSC, MPPSC एवं अन्य राज्य स्तरीय परीक्षाओं के लिए आकार IAS की सर्वोत्तम पाठ्यपुस्तकें और अध्ययन सामग्री।",
  path: "/publications",
  keywords: ["Aakar IAS Books", "MPPSC Books Hindi", "UPSC History Book", "MP GK Book", "Aakar IAS publications"],
});
...
export default async function PublicationsPage() {
  ...
  const categories = [
    ...
    { name: "इतिहास (History)", active: false },
    { name: "हिन्दी माध्यम", active: false },
    { name: "करेंट अफेयर्स", active: false },
  ];
        ...
        <Container size="wide" className="relative py-16 sm:py-20 flex flex-col md:flex-row items-center gap-10">
          <div className="flex-1 space-y-4">
            ...
            <p className="text-pretty text-lg text-white/75 max-w-xl">
              सिविल सेवा परीक्षाओं के लिए सर्वश्रेष्ठ अध्ययन सामग्री।
            </p>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <aside className="w-full lg:w-72 shrink-0 space-y-6">
              ...
              <div className="bg-card border border-border p-5 rounded-2xl shadow-soft">
                <h3 className="font-bold text-foreground mb-4 text-lg">श्रेणियां (Categories)</h3>
              ...
              <div className="bg-card border border-border p-5 rounded-2xl shadow-soft">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-bold text-foreground text-lg">अध्ययन संसाधन</h3>
                </div>
                <div className="space-y-3">
                  <Link href="/current-affairs" className="flex items-center gap-3 p-2 hover:bg-muted rounded-xl transition-all">
                    <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center text-primary shrink-0">
                      <BookOpen className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-foreground">डेली करेंट अफेयर्स</p>
                      <p className="text-xs text-muted-foreground">दैनिक अपडेट्स</p>
                    </div>
                  </Link>
                  <Link href="/free-pdf" className="flex items-center gap-3 p-2 hover:bg-muted rounded-xl transition-all">
                    <div className="w-10 h-10 rounded bg-primary/10 flex items-center justify-center text-primary shrink-0">
                      <FileText className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-foreground">मंथली पीडीएफ़</p>
                      <p className="text-xs text-muted-foreground">फ्री डाउनलोड</p>
                    </div>
                  </Link>
                </div>
            ...
            <div className="flex-1 space-y-12">
              ...
              {publications.length > 0 && (
                <section>
                  <h2 className="text-2xl font-extrabold text-foreground mb-6 border-l-4 border-primary pl-4">
                    मुख्य प्रकाशन (Featured Publications)
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {publications.slice(0, 2).map((pub) => (
                      <BookCard key={pub.id} pub={pub} locale="hi" />
                    ))}
                  </div>
                </section>
              )}
              ...
              {publications.length > 2 && (
                <section>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-2xl font-extrabold text-foreground">नवीनतम पुस्तकें (Latest Releases)</h2>
                  </div>
              ...
              {publications.length === 0 && (
                <div className="text-center py-16 border border-dashed rounded-2xl">
                  ...
                  <h3 className="font-bold text-lg text-foreground">कोई पुस्तक नहीं मिली</h3>
                  <p className="text-sm text-muted-foreground mt-1">शीघ्र ही नई पुस्तकें जोड़ी जाएँगी।</p>
                </div>
              )}
              ...
              {(faculties.length > 0 || experts.length > 0) && (
                <section className="pt-6">
                  <h2 className="text-2xl font-extrabold text-foreground mb-6 flex items-center gap-2">
                    <Users className="h-6 w-6 text-primary" /> हमारे विशेषज्ञ लेखक (Academic Experts)
                  </h2>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {(faculties.length > 0 ? faculties : experts.map((e, idx) => ({
                      ...
                      const name = f.nameHi || f.nameEn;
                      const title = f.titleHi || f.titleEn;
                      return (
          ...
          <Section className="mt-12 pb-0">
            <div className="bg-primary text-primary-foreground p-8 sm:p-12 rounded-2xl relative overflow-hidden shadow-soft-lg">
              ...
              <div className="relative z-10 max-w-2xl space-y-4">
                <h2 className="text-3xl font-extrabold text-white">नई किताबों की जानकारी सबसे पहले पाएं</h2>
