...
export default async function MppscNotesArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();
  ...
  const articleUrl = `${siteConfig.url}/mppsc-notes/${slug}`;
  const breadcrumb = breadcrumbJsonLd([
    { name: "Home", url: siteConfig.url },
    { name: "एमपीपीएससी नोट्स (MPPSC Notes)", url: `${siteConfig.url}/mppsc-notes` },
  ...
  return (
    <>
      ...
      <article className="min-h-screen pb-16">
        <ArticleHero article={article} locale="hi" />
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
              <aside className="hidden lg:block lg:col-span-3">
                <div className="sticky top-24 space-y-6">
                  ...
                  <ShareWidget title={article.title} url={articleUrl} />
                </div>
              </aside>

              <main className="lg:col-span-6 space-y-8">
                <ArticleBody article={article} locale="hi" />
              ...
              <aside className="lg:col-span-3 space-y-8">
                <div className="sticky top-24">
                  <ArticleSidebar related={recentArticles} locale="hi" ads={ads} />
                </div>
              </aside>
            </div>
          </Container>
        ...
        {recentArticles.length > 0 && (
          <Section className="bg-muted/30 border-t border-border py-12">
            <Container>
              <AnimatedSection>
                <RelatedArticles articles={recentArticles} locale="hi" />
              </AnimatedSection>
            </Container>
          </Section>
        )}
