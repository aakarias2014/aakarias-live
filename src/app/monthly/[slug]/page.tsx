...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("hi", "monthly");
...
export default async function MonthlyArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();
  const article = await repo.getArticle(slug, "hi");
  ...
  const breadcrumb = breadcrumbJsonLd([
    ...
    { name: "Monthly Current Affairs", url: `${siteConfig.url}/monthly` },
