...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("en", "editorial");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seoTitle = formatArticleSeoTitle(article.title, "en");
...
export default async function EnglishEditorialArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();
  const article = await repo.getArticle(slug, "en");
  ...
  const breadcrumb = breadcrumbJsonLd([
    ...
    { name: "Editorial", url: `${siteConfig.url}/en/editorial` },
