...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("en", "monthly");
...
export default async function EnglishMonthlyArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();
  const article = await repo.getArticle(slug, "en");
  ...
  const breadcrumb = breadcrumbJsonLd([
    ...
    { name: "Monthly Current Affairs", url: `${siteConfig.url}/en/monthly` },
