...
export const metadata: Metadata = buildMetadata({
  ...
  description: "Get latest exam notification timelines, total posts, eligibility criteria, apply dates, and official rulebook PDFs for MPPSC, MPESB, MPSI, and UPSC.",
...
export default async function EnglishNotificationsPage({ searchParams }: { searchParams?: Promise<{ category?: string }> }) {
  const resolvedParams = searchParams ? await searchParams : {};
  const selectedCategory = resolvedParams.category || "all";

  const repo = await getContentRepository();
  const allNotifications = await repo.listNotifications("en");
  ...
  const collectionPage = collectionPageJsonLd({
    ...
    description: "Get latest exam notification timelines, total posts, eligibility, and direct PDF downloads.",
    url: pageUrl,
    inLanguage: "en-IN",
    items: filteredNotifications.map((n) => ({
      name: n.title,
      url: n.slug ? `${siteConfig.url}/en/notifications/${n.slug}` : (n.url ?? ""),
    })),
  });
  ...
  return (
    <>
      <Section className="pb-0 pt-8 bg-muted/20 border-b border-border/50">
        <Container size="wide">
          <Breadcrumb items={[{ name: "Exam Notifications & Vacancies", href: "/en/notifications" }]} />
          <div className="mt-6 pb-8">
            ...
            <p className="mt-3 max-w-3xl text-base text-muted-foreground leading-relaxed">
              Official notification updates, application timelines, total vacancies, eligibility criteria, and direct PDF downloads for MPPSC, MPESB (Vyapam), MPSI, Patwari, SSC, and UPSC exams.
            </p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-2 pt-2">
              {categoryTabs.map((tab) => {
                ...
                const tabHref = tab.value === "all" ? "/en/notifications" : `/en/notifications?category=${tab.value}`;
        ...
        <Container size="wide">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
            ...
            <main className="lg:col-span-8 space-y-8">
                ...
                {filteredNotifications.length > 0 ? (
                              ...
                              <tr key={n.id || idx} className="hover:bg-sky-500/5 transition-colors duration-150">
                                ...
                                <td className="px-4 py-4 text-center align-middle">
                                  <Button
                                    ...
                                    asChild
                                  >
                                    <Link href={`/en/notifications/${n.slug || n.id}`}>
                                      View
                                    </Link>
                                  </Button>
                                </td>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center rounded-3xl border border-dashed border-border p-8 bg-card">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      There are no active exam notifications available for the selected category filter.
                    </p>
                  </div>
                )}
              </div>
              ...
              <CourseRecommendationCard locale="en" examCategory={selectedCategory === "upsc" ? "UPSC" : "MPPSC"} />
            </main>
            ...
            <aside className="lg:col-span-4 space-y-6">
              ...
              <div className="rounded-3xl border border-emerald-500/30 bg-emerald-500/10 p-6 shadow-soft space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-extrabold text-foreground text-base">Free WhatsApp Job Alerts</h4>
                    <p className="text-xs text-muted-foreground">MPPSC & Vyapam updates directly on WhatsApp</p>
                  </div>
                </div>

                <p className="text-xs text-muted-foreground leading-relaxed">
                  Join Aakar IAS official WhatsApp channel to receive instant notifications on new job openings, admit cards, and exam dates.
                </p>
              ...
              <div className="rounded-3xl border border-border bg-card p-6 shadow-soft space-y-4">
                ...
                <div className="space-y-2 text-sm">
                  <Link href="/en/mppsc/mains-syllabus" className="block rounded-xl p-3 hover:bg-muted font-bold text-foreground hover:text-primary transition-colors">
                    📖 MPPSC Mains New Syllabus 2026 & Pattern
                  </Link>
                  <Link href="/en/pyq" className="block rounded-xl p-3 hover:bg-muted font-bold text-foreground hover:text-primary transition-colors">
                    📄 MPPSC & UPSC Solved PYQ Bank (2015-2025)
                  </Link>
                  <Link href="/en/ncert-books" className="block rounded-xl p-3 hover:bg-muted font-bold text-foreground hover:text-primary transition-colors">
                    📚 NCERT Books List for MPPSC Preparation
                  </Link>
                  <Link href="/en/online-courses" className="block rounded-xl p-3 hover:bg-muted font-bold text-foreground hover:text-primary transition-colors">
