...
export const metadata: Metadata = buildMetadata({
  title: "Aakar IAS Media Center – Free Civil Services Video Classes",
  description: "Free video lectures, live classes, educational podcasts, and daily study short tricks for UPSC & MPPSC preparation by Aakar IAS.",
  path: "/en/media-center",
  locale: "en",
...
export default async function EnMediaCenterPage({ searchParams }: Props) {
  ...
  const stats = [
    { title: "Total Channels", val: "5", icon: <Tv className="h-6 w-6 text-primary" /> },
    { title: "Total Videos", val: statsData.videoCount, icon: <Play className="h-6 w-6 text-primary" /> },
    { title: "Subscribers", val: statsData.subscriberCount, icon: <ExternalLink className="h-6 w-6 text-primary" /> },
    { title: "Total Views", val: statsData.viewCount, icon: <Eye className="h-6 w-6 text-primary" /> },
  ];
  ...
  const selectedPlaylistName = playlistId ? playlists.find(p => p.id === playlistId)?.title || "Playlist Videos" : "";

  // Trending / Featured Video (Latest video from uploads)
  const featuredVideo = latestVideos[0] || {
    id: "dQw4w9WgXcQ",
    title: "Indian Constitution: An Introduction",
  ...
  return (
    <>
      <section className="border-b border-border bg-muted/20 py-4">
        <Container size="wide">
          <Breadcrumb items={[{ name: "Free Classes", href: "/en/media-center" }]} />
        </Container>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <aside className="lg:col-span-3 space-y-6 hidden lg:block">
              <div className="bg-card border border-border p-4 rounded-2xl space-y-1 shadow-soft">
                <p className="text-xs font-bold text-muted-foreground uppercase px-3 mb-2 tracking-wider">Navigation</p>
                <Link
                  href="/en/media-center?tab=home"
                  className={cn(
              ...
              <div className="bg-primary/5 border border-primary/10 p-5 rounded-2xl text-center space-y-4">
                ...
                <p className="text-xs text-muted-foreground">Unlock unlimited course guides and custom 1-on-1 mentorship programs.</p>
                ...
                <>
                  ...
                  <div className="bg-secondary rounded-3xl overflow-hidden flex flex-col md:flex-row relative shadow-lg">
                    ...
                    <div className="flex-1 p-8 sm:p-10 flex flex-col justify-center space-y-4 relative z-10 text-white">
                      ...
                      <div className="flex flex-wrap gap-3 pt-2">
                        <Link href={`/en/media-center?tab=home&v=${featuredVideo.id}`}>
                    ...
                    <div className="space-y-6">
                      <div className="flex justify-between items-end">
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">Connect directly on our YouTube channel</p>
                        </div>
                        <Link href="/en/media-center?tab=live" className="text-xs text-primary font-bold hover:underline flex items-center gap-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {liveSessions.slice(0, 2).map((live) => (
                          <Link key={live.id} href={`/en/media-center?tab=home&v=${live.id}`}>
                              ...
                              <div className="relative z-10 p-5 text-white space-y-2">
                                ...
                                {live.scheduledStartTime && (
                                  <div className="flex items-center gap-1 text-[10px] text-white/80 pt-1 border-t border-white/10">
                                    ...
                                    <span>Scheduled: {new Date(live.scheduledStartTime).toLocaleString("en-US")}</span>
                                  </div>
                                )}
                              </div>
                            </div>
                  ...
                  <div className="space-y-6">
                    <div className="flex justify-between items-end">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Most recent video classes for structured preparation</p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                      {latestVideos.slice(0, 6).map((video) => (
                        <Link key={video.id} href={`/en/media-center?tab=home&v=${video.id}`}>
                  ...
                  {shorts.length > 0 && (
                    <div className="space-y-6">
                      <div>
                        <h2 className="text-xl font-extrabold text-foreground border-l-4 border-primary pl-4">
                          Latest YouTube Shorts (Quick Revision)
                        </h2>
                        <p className="text-xs text-muted-foreground mt-1">Understand quick facts through bite-sized shorts</p>
                      </div>
                      <div className="flex gap-4 overflow-x-auto hide-scrollbar pb-3">
                        {shorts.map((short) => (
                          <Link key={short.id} href={`/en/media-center?tab=home&v=${short.id}`} className="flex-none w-44">
              ...
              {tab === "live" && (
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">List of scheduled and active YouTube Live sessions</p>
                  </div>

                  {liveSessions.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {liveSessions.map((live) => (
                        <Link key={live.id} href={`/en/media-center?tab=live&v=${live.id}`}>
                            ...
                            <div className="relative z-10 p-5 text-white space-y-2">
                              ...
                              {live.scheduledStartTime && (
                                <div className="flex items-center gap-1 text-xs text-white/80 pt-1 border-t border-white/10">
                                  ...
                                  <span>Start Time: {new Date(live.scheduledStartTime).toLocaleString("en-US")}</span>
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="text-center py-16 bg-card border border-dashed border-border rounded-2xl space-y-3">
                      ...
                      <h4 className="font-bold text-foreground">No Live Sessions Active</h4>
                      <p className="text-xs text-muted-foreground max-w-sm mx-auto">No live sessions are scheduled right now. Join our Telegram to stay updated.</p>
                    </div>
                  )}
                </div>
              )}
                  ...
                  {playlistId ? (
                    ...
                    <div className="space-y-6">
                      <div className="flex items-center gap-2">
                        <Link href="/en/media-center?tab=playlists" className="flex items-center gap-1 text-xs text-primary font-bold hover:underline">
                          <ChevronLeft className="h-4 w-4" /> Back to Playlists List
                        </Link>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">YouTube Video Lectures</p>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {selectedPlaylistVideos.map((video) => (
                          <Link key={video.id} href={`/en/media-center?tab=playlists&playlistId=${playlistId}&v=${video.id}`}>
                            <div className="group cursor-pointer space-y-3 bg-card border border-border p-3 rounded-2xl shadow-soft hover:shadow-soft-lg transition-all flex flex-col h-full justify-between">
                              <div className="space-y-3">
                                <div className="relative aspect-video rounded-xl overflow-hidden bg-muted border border-border/40">
                                  <Image src={video.thumbnail} alt={video.title} fill className="object-cover" />
                    ...
                    <div className="space-y-8">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">Structured course playlists for subject-wise preparation</p>
                      </div>

                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {playlists.map((playlist) => (
                          <Link key={playlist.id} href={`/en/media-center?tab=playlists&playlistId=${playlist.id}`}>
                              ...
                              <div className="space-y-3">
                                <div className="relative aspect-video rounded-xl overflow-hidden bg-muted border border-border/40">
                                  ...
                                  <div className="absolute bottom-2 right-2 bg-black/80 px-2 py-0.5 rounded text-[10px] font-bold text-white flex items-center gap-1">
                                    ...
                                    <span>{playlist.videoCount} classes</span>
                                  </div>
                                </div>
                                <h4 className="font-bold text-foreground text-sm line-clamp-2 leading-snug group-hover:text-primary transition-colors">
                                  {playlist.title}
                                ...
                                <p className="text-xs text-muted-foreground line-clamp-2">
                                  {playlist.description || "UPSC & MPPSC Examination Guidance"}
                                </p>
                              </div>
                            </div>
                          </Link>
              ...
              {tab === "podcasts" && (
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Topper talk, strategy, and student guidance series</p>
                  </div>
                  ...
                  {podcastPlaylists.length > 0 ? (
                    ...
                    <div className="text-center py-16 bg-card border border-dashed border-border rounded-2xl space-y-3">
                      ...
                      <h4 className="font-bold text-foreground">Podcasts Not Available Yet</h4>
                      <p className="text-xs text-muted-foreground max-w-sm mx-auto">Please wait for our upcoming podcast series.</p>
                    </div>
                  )}
                </div>
              )}
