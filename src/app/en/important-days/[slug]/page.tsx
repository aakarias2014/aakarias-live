...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("en", "currentAffairs");
...
export default async function EnglishImportantDaysArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();

  const [article, recentArticlesRes, ads] = await Promise.all([
    repo.getArticle(slug, "en"),
    repo.listArticles({ locale: "en", tag: "important-days", pageSize: 5 }),
    repo.listAds("en"),
  ]);
  ...
  const articleUrl = `${siteConfig.url}/en/important-days/${slug}`;
  const breadcrumb = breadcrumbJsonLd([
    { name: "Home", url: siteConfig.url },
    { name: "Current Affairs", url: `${siteConfig.url}/en/current-affairs` },
    { name: "Important Days", url: `${siteConfig.url}/en/important-days` },
  ...
  if (article.mcqs && article.mcqs.length > 0) {
    schemas.push(
      quizJsonLd({
        ...
        description: `Practice questions and detailed explanations on ${article.title}.`,
        url: articleUrl,
