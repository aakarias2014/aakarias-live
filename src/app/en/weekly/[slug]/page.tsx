...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("en", "weekly");
...
export default async function EnglishWeeklyArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();
  const article = await repo.getArticle(slug, "en");
  ...
  const breadcrumb = breadcrumbJsonLd([
    ...
    { name: "Weekly Current Affairs", url: `${siteConfig.url}/en/weekly` },
