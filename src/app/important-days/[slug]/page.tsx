...
export async function generateStaticParams() {
  ...
  const slugs = await repo.getAllSlugs("hi", "currentAffairs");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${article.title} | Important Days | Aakar IAS`,
    description: article.excerpt,
    path: `/important-days/${slug}`,
...
export default async function ImportantDaysArticlePage({ params }: Props) {
  const { slug } = await params;
  const repo = await getContentRepository();

  const [article, recentArticlesRes, ads] = await Promise.all([
    repo.getArticle(slug, "hi"),
    repo.listArticles({ locale: "hi", tag: "important-days", pageSize: 5 }),
    repo.listAds("hi"),
  ]);
  ...
  const articleUrl = `${siteConfig.url}/important-days/${slug}`;
  const breadcrumb = breadcrumbJsonLd([
    { name: "Home", url: siteConfig.url },
    { name: "Current Affairs", url: `${siteConfig.url}/current-affairs` },
    { name: "महत्वपूर्ण दिवस (Important Days)", url: `${siteConfig.url}/important-days` },
  ...
  if (article.mcqs && article.mcqs.length > 0) {
    schemas.push(
      quizJsonLd({
        ...
        description: `महत्वपूर्ण दिवस से संबंधित अभ्यास बहुविकल्पीय प्रश्न और व्याख्या।`,
        url: articleUrl,
